...
import { getCourseProgress, readProgress, toggleLessonProgress } from "./lib/progress";
import { getDocumentTitle, readRoute } from "./lib/routing";
...
function MenuIcon({ close = false }) {
  const path = close ? "M5 5l14 14M19 5 5 19" : "M4 7h16M4 12h16M4 17h16";
  return <svg aria-hidden="true" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d={path} /></svg>;
}
...
function ProgressBar({ course, progress, compact = false }) {
  ...
  return <>
    ...
    <div className="progress-meta"><span>{current.completed} of {current.total} lessons complete</span>{!compact && <span>{current.percent}%</span>}</div>
  </>;
}

function CourseCard({ course, progress }) {
  return <article className="course-card">
    <div className="course-card-topline"><span className={`tag tag-${course.area.toLowerCase()}`}>{course.label}</span><span className="course-length">{course.lessons.length} lessons</span></div>
    <h3 className="course-title">{course.title}</h3>
    <p className="course-summary">{course.summary}</p>
    <div className="course-card-footer"><span>{course.level}</span><a className="text-button" href={`#/study/${course.id}/0`}>Open course <ArrowIcon /></a></div>
  </article>;
}
...
function Header({ page, menuOpen, onToggleMenu }) {
  ...
  return <header className="site-header"><div className="header-inner">
    ...
    <nav className={`primary-nav${menuOpen ? " is-open" : ""}`} id="primary-nav" aria-label="Primary navigation">{navItems.map(([key, label]) => <a key={key} className="nav-link" href={`#/${key}`} aria-current={page === key ? "page" : undefined}>{label}</a>)}</nav>
    <div className="header-actions"><a className="identity-chip" href="#/dashboard" aria-label="Open study dashboard"><span className="identity-initials">SP</span><span>Study space</span></a><button className="menu-button" type="button" onClick={onToggleMenu} aria-label={`${menuOpen ? "Close" : "Open"} navigation menu`} aria-controls="primary-nav" aria-expanded={menuOpen}><MenuIcon close={menuOpen} /></button></div>
  </div></header>;
}
...
function HomePage({ progress }) {
  return <>
    <section className="hero"><div className="page-section hero-grid">
      <div className="hero-copy"><span className="eyebrow">SP Institute</span><h1 className="display-title" id="page-title" tabIndex="-1">Professional learning that respects your attention.</h1><p className="lede">Read structured courses in business and programming at your own pace. Every course is organized for direct study, with clear sections, practical context, and no distractions competing for your time.</p><div className="button-row"><a className="button button-primary" href="#/catalog">Explore the catalog <ArrowIcon /></a><a className="button button-secondary" href="#/dashboard">Open study dashboard</a></div></div>
      <aside className="hero-panel" aria-label="Study overview"><div className="panel-topline"><span>Study workspace</span><span className="status-dot">Available</span></div><div className="continue-card"><span className="tag tag-business">Business strategy</span><h2>{courses[0].title}</h2><p>Continue with a structured lesson on turning direction into operating priorities.</p><div style={{ marginTop: "1rem" }}><ProgressBar course={courses[0]} progress={progress} /></div><div className="button-row"><a className="button button-primary button-small" href="#/study/strategy-systems/0">Start reading <ArrowIcon /></a></div></div><div className="panel-stat-grid"><div className="panel-stat"><strong>{courses.length}</strong><span>Available courses</span></div><div className="panel-stat"><strong>2 areas</strong><span>Business and programming</span></div></div></aside>
    </div></section>
    <section className="page-section"><div className="trust-row" aria-label="Learning format">{["Read-only learning", "Responsive study layout", "Clear progress controls"].map((item) => <span className="trust-item" key={item}><span className="trust-check">✓</span>{item}</span>)}</div><div className="section-heading mt-section"><div><span className="eyebrow">Start here</span><h2 className="section-title" style={{ marginTop: ".6rem" }}>Courses designed for practical use.</h2><p className="lede">Choose a subject, read one lesson at a time, and return to the point where you left off in this browser.</p></div><a className="text-button" href="#/catalog">View all courses <ArrowIcon /></a></div><div className="course-grid">{courses.slice(0, 3).map((course) => <CourseCard key={course.id} course={course} progress={progress} />)}</div><div className="metric-grid mt-section" aria-label="Platform capabilities">{[["Focused", "One reading experience, without video or assignment workflows."], ["Responsive", "Layout adapts to phones, tablets, laptops, and larger screens."], ["Expandable", "Courses use one shared data model so the catalog can grow cleanly."]].map(([title, copy]) => <div className="metric" key={title}><strong>{title}</strong><span>{copy}</span></div>)}</div><div className="section-heading mt-section"><div><span className="eyebrow">How it works</span><h2 className="section-title" style={{ marginTop: ".6rem" }}>A calm path through a complex subject.</h2></div></div><div className="path-grid">{[["01", "Choose a course", "Browse by subject area or search for a topic that matches the work you need to do."], ["02", "Read with context", "Move through short, ordered lessons that explain a subject without requiring a live session."], ["03", "Track your place", "Mark a lesson complete and use the dashboard to return to the next piece of reading."]].map(([number, title, copy]) => <article className="path-card" key={number}><span className="path-number">{number}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></section>
  </>;
}
...
function CatalogPage({ topic, query, progress, onTopicChange, onQueryChange }) {
  ...
  return <>
    ...
    <section className="page-section"><div className="catalog-layout"><aside className="catalog-filter" aria-label="Course filters"><div><label className="filter-label" htmlFor="course-search">Search courses</label><div className="search-wrap"><span className="search-icon"><SearchIcon /></span><input className="search-input" id="course-search" type="search" placeholder="Search a subject" autoComplete="off" value={query} onChange={(event) => onQueryChange(event.target.value)} /></div></div><div className="filter-group"><span className="filter-label">Subject area</span><div className="filter-options" role="group" aria-label="Filter courses by subject area">{courseAreas.map((area) => { const count = area === "All" ? courses.length : courses.filter((course) => course.area === area).length; return <button className="filter-option" type="button" key={area} onClick={() => onTopicChange(area)} aria-pressed={topic === area}><span>{area === "All" ? "All courses" : area}</span><span className="filter-count">{count}</span></button>; })}</div></div></aside><div><div className="catalog-results-header"><h2 className="section-title">Available courses</h2><span className="result-count">{matches.length} {matches.length === 1 ? "course" : "courses"}</span></div>{matches.length ? <div className="catalog-course-grid">{matches.map((course) => <CourseCard key={course.id} course={course} progress={progress} />)}</div> : <div className="empty-state"><div><h2>No courses match that search.</h2><p>Try a different word or choose another subject area.</p></div></div>}</div></div></section>
  </>;
}
...
function DashboardPage({ progress }) {
  const totals = courses.reduce((result, course) => {
    const current = getCourseProgress(course, progress);
    return { completed: result.completed + current.completed, lessons: result.lessons + current.total };
  ...
  return <>
    ...
    <section className="page-section"><div className="dashboard-top"><div><h2 className="section-title">Continue studying</h2><p className="lede" style={{ marginTop: ".55rem" }}>Your current browser has {totals.completed} completed of {totals.lessons} available lessons.</p></div><a className="button button-secondary" href="#/catalog">Browse catalog <ArrowIcon /></a></div><div className="dashboard-grid"><section className="dashboard-panel" aria-labelledby="active-courses-title"><div className="panel-heading"><h2 id="active-courses-title">Courses in your study space</h2><span className="tag">Read only</span></div><div className="learning-list">{courses.slice(0, 4).map((course) => { const current = getCourseProgress(course, progress); const nextLesson = Math.min(current.completed, course.lessons.length - 1); return <article className="learning-row" key={course.id}><div><h3>{course.title}</h3><p>{course.area} · {course.lessons.length} lessons</p><ProgressBar course={course} progress={progress} compact /></div><a className="button button-secondary button-small" href={`#/study/${course.id}/${nextLesson}`}>{current.completed ? "Continue" : "Start"} <ArrowIcon /></a></article>; })}</div></section><aside className="dashboard-aside"><section className="dashboard-aside-card"><h2>Study format</h2><p>Courses are designed for reading. This site does not use video, teacher-led sessions, graded assignments, social groups, or leaderboard mechanics.</p><div className="availability-list">{["One consistent study interface", "Native keyboard controls", "Responsive reading width"].map((item) => <span className="availability-item" key={item}>{item}</span>)}</div></section><section className="dashboard-aside-card"><h2>Progress storage</h2><p>This GitHub Pages release stores lesson completion in the browser that is currently in use. A connected account service is required before progress can be shared between devices.</p></section></aside></div></section>
  </>;
}
...
export default function App() {
  ...
  useEffect(() => {
    setMenuOpen(false);
    document.title = getDocumentTitle(route.page);
  ...
  function toggleComplete(courseId, lessonIndex) {
    setProgress((current) => toggleLessonProgress(current, courseId, lessonIndex));
  }
